...
| D017 | M003/S03/T03 | architecture | Finalize Findings as single action replacing separate freeze + generate report | Single "Finalize Findings" action that freezes research checkpoint and triggers report generation atomically | User intent is always "I'm done researching, make my report" — a separate Generate Report button creates an unnecessary extra step and prop chain through 4 components. Combining freeze + generate into one action simplifies the UX and removes 5 lines of prop threading. | Yes | agent |
| D018 |  | requirement | R057 | validated | Report workspace feedback + regeneration fully implemented in M003/S04. Orchestrator accepts optional feedback param (4 tests), store persists reportFeedback with hydration (6 tests), ReportWorkspace UI shows feedback textarea + Regenerate/Done buttons (7 tests). Full suite 711/711 pass, build succeeds. | Yes | agent |
| D019 | M003/S06 | testing | Browser verification uses proxy mode with server-side API keys from .env.local | Use browser automation via proxy mode — dev server reads API keys from .env.local server-side via buildProviderConfigs(), no client-side key entry needed during verification | User override: S06 should use browser automation for verification. The app already supports proxy mode where keys come from env vars server-side. This avoids automating the settings UI to enter keys manually and matches the production deployment pattern. | Yes | human |
| D020 |  | requirement | R058 — Report export Markdown download | validated | M003/S05 T01 — downloadBlob utility + FinalReport export dropdown with "Download Markdown" option. 3 unit tests for download utility, 732/732 full suite pass, build clean. | Yes | agent |
| D021 |  | requirement | R059 — Report export PDF generation (client-side) | validated | M003/S05 T01 — exportReportAsPdf using html2pdf.js with marked for MD→HTML. 8 unit tests for PDF export, 732/732 full suite pass, build clean. | Yes | agent |
| D022 |  | requirement | R060 — Search result export (MD/JSON) | validated | M003/S05 T02 — serializeSearchResultAsMd and serializeSearchResultsAsJson helpers + per-card export dropdown in SearchResultCard. 10 unit tests, 732/732 full suite pass, build clean. | Yes | agent |
| D023 |  | requirement | R061 — Add-to-knowledge-base from search results | validated | M003/S05 T02 — searchResultToKnowledgeItem converter + Database icon button on SearchResultCard wired to knowledgeStore.add() with success toast. 10 unit tests including KB conversion, 732/732 full suite pass. | Yes | agent |
| D024 | M004 | architecture | Time budget same across all deployment targets (Docker, Vercel, local dev) | Same 2-cycle, 180s budget everywhere. No deployment-target branching. | Simpler mental model, same UX on all platforms. Docker users just get more auto-reconnects. No env-based config branching needed. | Yes | human |
| D025 | M004 | api | Full pipeline (phase=full) removal strategy | Clean removal of fullSchema, handleFullPhase(), useResearch().start(), and Phase type "full" entry. | No external API consumers depend on the full pipeline. Deprecation shim would be dead code to maintain. The multi-phase flow is the only viable pattern for serverless. | Yes | human |
| D026 | M004 | ux | Auto-review UX — visible state with round progress | Visible "Auto-review round N/M..." state with progress indicator, user can abort. | Users need to know what the system is doing and maintain control. Silent auto-review would feel like the app is stuck or doing something unexpected. | Yes | human |
| D027 | M004/S01 | architecture | Research batching strategy for Vercel Hobby timeout compliance | Cycle cap (maxCyclesPerInvocation=2) as primary control, timeBudgetMs=180s as safety net backup | Each search-analyze cycle takes ~80s. 2 cycles ≈ 160s, well within Vercel Hobby's 300s limit. Time budget alone was unreliable because cycle durations vary — cycle cap provides deterministic batching. The 180s time budget catches edge cases where individual cycles run long. | Yes | collaborative |
| D028 | M004/S01 | architecture | Review as standalone SSE phase vs embedded in research phase | Dedicated review phase with review-result event type, phase='review' in SSE request | Separating review into its own phase gives it independent timeout budgeting, distinct SSE event type for store handling, and clean API boundary. The review phase takes plan + learnings + sources + images + optional suggestion as input — a completely different schema from research. Embedding would require conditional logic in the research handler and ambiguous event types. | Yes | collaborative |
| D029 | M004/S01 | pattern | Auto-review trigger mechanism in useResearch hook | useEffect watching researchState + autoReviewRoundsRemaining for auto-review trigger | Using a reactive useEffect watching store state is simpler and more debuggable than intercepting SSE events directly. The counter pattern (autoReviewRoundsRemaining) is persisted, so review resumes correctly after page refresh. The effect fires when state transitions to awaiting_results_review AND rounds remain, providing a clean trigger point. | Yes | agent |
