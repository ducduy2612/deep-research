...
- [x] **T02: Create ReportWorkspace UI with feedback textarea, Regenerate/Done buttons; wire into PhaseAccordion; guard auto-navigation with report checkpoint** — Create ReportWorkspace component (~180 lines) with MarkdownRenderer for streamed report, feedback textarea, Regenerate button (calls regenerateReport), Done button (calls freeze('report') then navigates). Wire into PhaseAccordion via onRenderReport in ActiveResearchCenter. Guard auto-navigation in page.tsx to only navigate when report checkpoint exists. Add all i18n strings.
  - Estimate: 1.5h
